...
class FormParser(HTMLParser):
    ...
    def department_intake_values(self) -> dict[str, str]:
        values: dict[str, str] = {}
        for item in self.inputs:
            name = item.get("name", "")
            if not name.startswith("department_intake_"):
                continue
            input_type = item.get("type", "text")
            values[name] = "on" if input_type == "checkbox" else "Smoke intake value"
        for name in self.options:
            if name.startswith("department_intake_"):
                values[name] = self.first_option_value(name)
        return values
...
def submit_ticket(client: SmokeHttpClient) -> int:
    ...
    fields.update(form.department_intake_values())
